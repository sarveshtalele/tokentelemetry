...
export function SessionCard({ s, onClick }: { s: SessionRow; onClick?: () => void }) {
  ...
  return (
    <div onClick={onClick} className={`bg-surface-muted rounded-lg p-4 ${onClick ? 'cursor-pointer hover:bg-slate-100' : ''}`}>
      <div className="font-mono text-xs text-slate-500">{s.session_id.slice(0, 12)}</div>
